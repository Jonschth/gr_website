...
---
tile: 'Golden Redfish in Icelandic waters: a statistical approach'
format: 
    html:
        code-fold: true
jupyter: python3
...
### Data fetched and heat maps generated
...
The regression is set up in the normal fashion: y = f(X), where y, the independant variable is the stock index and X, the dependent variable is the set of observation and f is a function derived from the regression.
...
def plot_result_range(result_dict, interval_int, fractile, y_offset):
    ...
    """
    result_dict['x'] = range(343+y_offset,
                             453+y_offset,
...
### Free regression (with slack variables for the period 2014 to 2019)
...
### Running code and results, using random training and testing data - instance 1.


  1. Heatmap showing the relationship between length and weight through the years
  2. Adjust data (cf. appendix 1) 
...
The optimal solutions (the solutions that best balances the mean average error ("mae") and explainability ($r^2$) ) is that there are currently 385, 395 or 412 m individuals. At this point mae $ \approx $ 52m individuals and $r^2$ $ \approx $ 0.55. This is a good estimation although we would have hoped for a lower mae.
  
We do two kind of shap analysis. The first figures shows the 



that the length classes that are most important in reducing the size of the stock are the classes between 29-36 cm as we would expect. The number of individuals that have been caught by the fleet (the code starting with "10") is also very important.

The Max(cum) variable is of major importance as it contains the CPU of the respective survey (i.e. number of individuals caught).
...
### Running code and results, using fixed testing data and slack variables - instance 2.


  1. Adjust and fetch data(cf appendix 1)
  2. Show the heat maps
  3. Run "free" gradient boosting
  4. Regress in a loop over possible values 
  5. Shap analysis
    
As we were dissatisfied with the high mean average error in our initial approach we wanted conducted more analysis. Given the rapid decline in stocks in the past few years we were keen to test if the estimates of the past 10 years may have been excessive. By introducing slack variables for the years 2012 - 2019 and fixing the test on the same period as well as fixing the result of the current 2 years we got a much better result. In this case the current stock size was estimated at 380 m individuals. At this point mae $ \approx $ 40m individuals and $r^2$ $ \approx $ 0.8.
...
The shap analysis shows that the length classes that are most important in reducing the size of the stock are the classes between 29-36 cm as we would expect. The amount caught by the fleet in the same length groups is also very important in this running instance.

 
The Max(cum) variable is even more important in this instance. This is to be expected as this run is more precise and therefore more sensitive than the previous instance.
...
y_offset = int((y.iloc[37]-y_2022)/1e6)

result_dict_gb = regression_over_possible_values_XGB(X,
                                                     y,
                                                     interval_int)
plot_result_range(result_dict_gb, interval_int, fractile, y_offset)
```
...
![The figure shows the age distribution without correcting for large catches](attachment:62f0ee65-fce7-4070-93b2-fd5e81750c33.png){width=80%}



![The figure shows the age distribution where 96% fractile correction has been employed. Fractile levels were calibrated by examining different fractile levels between 90 and 100%,](attachment:05a16beb-152d-40a0-befe-a74ca37ae1d5.png){width=80%}
